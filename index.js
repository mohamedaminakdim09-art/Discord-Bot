...
const commands = [
 ...
 .setDescription('Chouf les commandes kamlin'),
  ...
  new SlashCommandBuilder()
 .setName('join')
 .setDescription('Dkhel l\'voice channel li nta fih'),

  new SlashCommandBuilder()
 .setName('leave')
 .setDescription('Khrej men l\'voice channel'),
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName === 'help') {
   ...
   .addFields(
        ...
        { name: '🎤 /join', value: 'Dkhel l\'voice channel' },
        { name: '👋 /leave', value: 'Khrej men l\'voice channel' },
        { name: '❓ /help', value: 'Had l\'liste' },
        { name: '🖼 /avatar', value: 'Chouf l\'avatar dyal user' },
  ...
  if (interaction.commandName === 'join') {
    ...
    if (!voiceChannel) {
      return interaction.reply({ content: '❌ Khassk tkon f chi voice channel a sat!', ephemeral: true });
    }
    ...
    try {
      ...
      console.error(error);
      await interaction.reply({ content: '❌ Ma 9derch ndkhel l\'voice', ephemeral: true });
    }
  }
  ...
  if (interaction.commandName === 'leave') {
    ...
    if (!connection) {
      return interaction.reply({ content: '❌ Ma kntch f voice aslan a sat', ephemeral: true });
    }

    connection.destroy();
    await interaction.reply('👋 Khrejt men l\'voice channel');
  }
